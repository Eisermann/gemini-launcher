...
class GeminiTerminalManager(private val project: Project) {
    ...
    companion object {
        private val GEMINI_TERMINAL_KEY = Key.create<Boolean>("gemini.link.geminiTerminal")
        private val GEMINI_TERMINAL_RUNNING_KEY = Key.create<Boolean>("gemini.link.geminiTerminal.running")
        private val GEMINI_TERMINAL_CALLBACK_KEY = Key.create<Boolean>("gemini.link.geminiTerminal.callbackRegistered")
        private val GEMINI_TERMINAL_SHIFT_ENTER_KEY = Key.create<Boolean>("gemini.link.geminiTerminal.shiftEnterRegistered")
    }
    ...
    private val scriptFactory = CommandScriptFactory(
        ...
        supportsPosixShell = {
            if (!SystemInfoRt.isWindows) {
                ...
                runCatching { service<GeminiLinkSettings>().state.winShell == WinShell.WSL }.getOrDefault(false)
            }
        }
    )
