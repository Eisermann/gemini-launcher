...
class DiscoveryService : Disposable {
    ...
    fun createDiscoveryFile(port: Int) {
        try {
            ...
            val codexIdeDir = Path.of(tmpDir, "codex", "ide")

            if (!Files.exists(codexIdeDir)) {
                Files.createDirectories(codexIdeDir)
            }

            val fileName = "codex-ide-server-$pid-$port.json"
            val file = codexIdeDir.resolve(fileName).toFile()
