...
class CodexTerminalManager(private val project: Project) {
    ...
    companion object {
        private val CODEX_TERMINAL_KEY = Key.create<Boolean>("codex.link.codexTerminal")
        private val CODEX_TERMINAL_RUNNING_KEY = Key.create<Boolean>("codex.link.codexTerminal.running")
        private val CODEX_TERMINAL_CALLBACK_KEY = Key.create<Boolean>("codex.link.codexTerminal.callbackRegistered")
        private val CODEX_TERMINAL_SHIFT_ENTER_KEY = Key.create<Boolean>("codex.link.codexTerminal.shiftEnterRegistered")
    }
    ...
    private val scriptFactory = CommandScriptFactory(
        ...
        supportsPosixShell = {
            if (!SystemInfoRt.isWindows) {
                ...
                runCatching { service<CodexLinkSettings>().state.winShell == WinShell.WSL }.getOrDefault(false)
            }
        }
    )
